...
class YoloNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.infer_hz = self.get_parameter("infer_hz").get_parameter_value().double_value
        self._min_dt_ns = 0
        if self.infer_hz and self.infer_hz > 0.0:
            self._min_dt_ns = int(1e9 / self.infer_hz)
    ...
    def image_cb(self, msg: Image) -> None:
        ...
        if self.enable:
              # --- throttle (infer_hz) ---
            if self._min_dt_ns > 0:
                now_ns = self.get_clock().now().nanoseconds
                if (now_ns - self._last_infer_ns) < self._min_dt_ns:
                    return
