...
You can also use the example launch script, which by default points to
`examples/demo_talker_listener_bridge.yaml` so it can immediately bridge
`demo_nodes_cpp talker` on domain 123 to `demo_nodes_py listener` on domain 125:

```xml
ros2 launch domain_bridge domain_bridge.launch.xml
```

You can provide a different config path if needed and override domain IDs with
optional launch arguments `from_domain` and `to_domain`.
